          ...
          >
            ...
            <div className="space-y-2">
              {insights.messages.map((msg, idx) => (
                <p key={idx} className="text-sm" style={{ color: 'rgb(var(--muted))' }}>
                  • {msg}
                </p>
              ))}
              {insights.cutSuggestions.length > 0 && (
                <div className="mt-3 pt-3" style={{ borderTop: '1px solid rgb(var(--border))' }}>
                  <p className="text-sm font-semibold mb-2" style={{ color: 'rgb(var(--text))' }}>
                    To balance your budget, consider cutting:
                  </p>
                  {insights.cutSuggestions.map((sug, idx) => (
                    <p key={idx} className="text-sm" style={{ color: 'rgb(var(--muted))' }}>
                      • {sug.categoryName}: ${sug.suggestedCut.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                    </p>
                  ))}
                </div>
              )}
